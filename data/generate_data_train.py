...
points=meshio.read("data/Stanford_Bunny_3d.mesh").points

triangles=meshio.read("data/Stanford_Bunny_3d.mesh").cells_dict['triangle']
np.random.seed(0)
...
for i in trange(600):
    ...
    ffd.array_mu_x=torch.tensor(diff_x)
    ffd.array_mu_y=torch.tensor(diff_y)
    ffd.array_mu_z=torch.tensor(diff_z)
    def_points=ffd(torch.tensor(points))    
